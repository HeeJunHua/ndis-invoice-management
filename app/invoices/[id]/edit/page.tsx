...
export default function EditInvoicePage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const invoice = await apiClient.get<Invoice>(`/api/invoices/${invoiceId}`);
  ...
  async function onRateSetChange(rateSetId: number | undefined) {
    ...
    try {
      const [cats, items] = await Promise.all([
        apiFetch<Category[]>(`/api/rate-sets/${rateSetId}/categories`),
        apiFetch<SupportItem[]>(`/api/rate-sets/${rateSetId}/support-items`),
      ]);
      setCategories(cats);
      ...
      form.setFieldsValue({
        items: (form.getFieldValue('items') || []).map((item: any) => ({
          ...item,
  ...
  async function onFinish(values: any) {
    ...
    try {
      ...
      await apiFetch(`/api/invoices/${params.id}`, { method: 'PATCH', body: JSON.stringify(payload) });
  ...
  return (
    <div style={{ padding: 24, maxWidth: 800 }}>
      <h1>Edit Invoice</h1>
      <Form form={form} layout="vertical" onFinish={onFinish} initialValues={{ status: 'drafted' }}>
